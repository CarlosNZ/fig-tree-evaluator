...
- **Releasing is manual, through `pnpm release`** (codegen/release.mjs; Carl, September 2026, Phase-14 review). It prompts for the next version with suggestions, stops unless CHANGELOG.md has an entry for it, bumps package.json and `src/version.ts`, runs what CI runs, commits and tags `vX.Y.Z`, and publishes with `npm publish` under `beta` for a `-beta.N` version or `latest` otherwise. Nothing is pushed. `pnpm release --dry-run` runs every step but makes no commit or tag, publishes with `npm publish --dry-run`, and restores package.json and `src/version.ts` afterwards. v2 patch releases are cut by hand from the `v2.x` branch, which does not carry the script.
