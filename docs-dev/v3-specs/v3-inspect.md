...
- **`x.inspect(options?)`**, the method, is convenience over the same function, and replaces the handle's `prettyPrint()` stand-in. It stays tree-shakeable by injection (Carl): the host passes the function to the constructor — `new FigTree(options, inspect)` — the handle takes it with the rest of its snapshot at compile, and the method calls it with itself. Without it the method throws a `FigTreeError` saying how to enable it. The method is always present on the type, since the class type cannot follow a constructor argument without more generics than it is worth.
- **`options` is `CallOptions`** in both forms, the per-call shape `validate()` and `evaluate()` take, laid over the options the handle pinned. As with `validate()`, only `data` bears on the report: it drives the sample-data check (see `issues`).
...
_Step 1 built (September 2026): the standalone `inspect()`, in `src/inspect/` — `index.ts` holds the function, the report's types and the `options` and `dependencies` renderings, `nodes.ts` the `canonicalForm` tree, `values.ts` the conversion rules. The report for the example above is exactly the one shown there, key for key._
...
- **Step 2, the method** — `x.inspect()` by injection, as recorded under "Surface". Built once step 1 is committed.
- **The dev [src/dev/inspect.ts](../../src/dev/inspect.ts)**: a thin console-printing wrapper over the report, or replaced.
- **Records to update** once the design closes: #156 (decision 1, and its line that #149 "already assumes the name" — #149's `prettyPrint()` is `FigTreeError`'s); "compile()" in [v3-evaluator-methods.md](v3-evaluator-methods.md), which settles `x.prettyPrint(options?)` and rules it "always a rendering, never a serialization"; the stand-in in [src/FigTree.ts](../../src/FigTree.ts) and its test.
