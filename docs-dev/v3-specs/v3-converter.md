...
- a fragment argument that the definition does not have, that loses to its unprefixed spelling, or, with `evaluateFullObject` on, that the body shadows.
...
A request the mock or the recordings cannot answer is a runner error, and stops the run. It is never a case outcome, since both engines failing in the same way would otherwise count as a match. Each double still fails the request, as the Jest mocks always have, but an engine catches that failure, and a fallback may swallow it, so the doubles also report it to a listener the runner checks after each case.
