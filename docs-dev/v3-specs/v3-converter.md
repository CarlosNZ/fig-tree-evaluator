...
_Status: **Agreed** (September 2026, signed off by Carl at Phase-15 planning). The other specs were changed as "Changes to other specs" lists, as 15.1's first chunk. Built so far: the placeholder `./migrate` entry, the reference tables ("The v2 reference table", 15.1's second chunk), stage 1 ("Stage 1: normalize", 15.1's third chunk), stage 2's frame with batch 1 (15.1's fourth chunk), batches 2 to 5 with the rest of the frame (15.1's fifth chunk), fragments, definitions and calls (15.1's sixth chunk), and the contract and the surface (15.1's seventh chunk), which completes the converter._
...
Both are pure functions over their arguments and the converter's own embedded v2 table ("Ruling: `migrateV2Expression` is a pure function carrying its own v2 tables" in [v3-migration.md](v3-migration.md), which stands). They take no `FigTree` and import nothing from the engine at runtime.

Converting a call needs the fragments' catalogue ("Fragments"), which walks every definition's body, so `migrateV2Expression` keeps the last catalogue it found, keyed by the JSON of the options it read. A script converts its expressions against one options object, and pays for the catalogue once: an expression against 40 fragments took 6.2 ms with the catalogue found at every call, and 0.24 ms with it kept (measured, September 2026). The key is the options' content, so options changed between calls are read afresh, and options holding anything JSON cannot hold, such as `undefined` or a Date, or an object with its own `toJSON`, which writes something other than what it holds, are never kept. The kept catalogue changes how long a call takes and nothing else, so the functions stay pure to their callers (ruled at 15.1's seventh chunk). So `./migrate` imports types only from the root, as `./editor-hints` does, and the same lint rule enforces it. Checking that the output validates is the caller's job: the page and the script both have an instance.
...
- **[v3-operator-parameters.md](v3-operator-parameters.md), "v2 disposition" for `plus`:** `type` → `expect` holds for `'array'`. `type: 'string'` converts to `join` with `delimiter: ''` instead, since v2 coerced the operands to text and `expect: 'string'` would reject the numbers it was used to concatenate, and `type: 'number'` converts the result, since v2 coerced no operand for it ("Batch 2").
...
| `noShorthand`        | On, `$name` keys were never shorthand, so an object carrying one is data. v2 itself ignored the option in `evaluateObject`, which expanded shorthand in GET and POST `parameters` and `headers`, GRAPHQL `variables` and CUSTOM_FUNCTIONS `input` anyway. The converter applies it everywhere. With `evaluateFullObject` on as well, v2 expanded a `$name` key in a plain object into a node, which it returned unevaluated, as data; the converter reads the key as an alias definition, as `evaluateFullObject` has it, and that goes to the guide. |
...
Each key is read as v2 read it: the flags by truthiness, `caseInsensitive` and `useCache` through `??`, and `fragments` and `functions` only in a shape v2 could use, any other counting as none. So a malformed option never reaches a rule.
...
- **Undeclared keys** are not in the table, since only MATCH read them, as branches from the node itself, and the normalizer gathers MATCH's into `branches` directly (stage 1, step 7). Most operators ignored such keys. A call that names a function in `operator` also had its undeclared keys gathered into `input` ([helpers.ts:121](../../v2-src/helpers.ts#L121)), but that is how v2 rewrote the call (stage 1, step 3), not how CUSTOM_FUNCTIONS behaved: its explicit form ignored undeclared keys.
...
The modifiers attach to whatever it returns. Where that is not a node (a reference or a constant cannot carry `fallback`), "Rules that cut across operators" decides what happens. A `build` may also take over a modifier: OBJECT_PROPERTIES turns a `fallback` into `get`'s `default`.

The **context** gives a rule `issue(tag, message)`, the node's source path, and the `V2Options`.

**A rule reads converted values.** A value v2 computed (a node, an alias or a fragment placeholder) may arrive as a string, since it converts to a reference: `'$vars.p'`, `'$params.p'`, `'$data.key'`. A v2 literal never arrives in that form, since the `literal` wrap quotes any text v3 would read as a reference. So a rule that treats a string as literal text asks `isComputed` first, which reads a reference string as computed (ruled at the PR #190 review, where three rules had read one as text).
...
v2's `type` did up to two jobs. For `'string'` and `'array'` it coerced the operands before adding them. And because v2 read `outputType ?? type` ([evaluate.ts:210](../../v2-src/evaluate.ts#L210)), every value of it also converted the result, which for `'number'` and `'boolean'` was all it did: v2's metadata describes `type` as coercing the operands, but PLUS coerces only for the other two ([PLUS/operator.ts:22-24](../../v2-src/operators/PLUS/operator.ts#L22-L24)), and v2's README documents only those two. `plusType` handles both:
...
| `'number'`                   | `plus`, with `outputType: 'number'` handed to the modifier rule                | v2 added as usual, then converted the result: `['5', '6']` concatenated to `'56'`, and gave `56`. `expect: 'number'` would reject that text (ruled at 15.2's fourth chunk) |
...
| `{ operator: '+', values: ['a', 'b'], type: 'number' }`   | `0`        | failure               | `output-type` issue                 |
...
The other measured cases give the same answer in both: the four `plus` modes, a single operand, `type: 'string'` over numbers, booleans and floats, `type: 'array'` over mixed and nested operands, `type: 'number'` over numbers and numeric text, `type: 'boolean'`, `type` beside `outputType`, both spellings of SUBTRACT and DIVIDE and the two together, `'quotient'` over negative values, and `'remainder'` where the signs agree.
...
**OBJECT_PROPERTIES.** "v2 disposition" for `get` in [v3-operator-parameters-2.md](v3-operator-parameters-2.md) rules the rename (`property` → `path`) and the pairing of a `fallback` with `get`'s `default`. `preferReference` writes the everyday case as a reference, and the rest as `get`:

| v2 node                                                       | Becomes                                                                            |
| ------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| a literal path, and nothing else                              | `"$data.<path>"`; an empty path is `"$data"`                                       |
| with a `fallback`                                             | `get` with `default: <the fallback>`, which the rule takes over from the modifiers |
...
- **`outputType`** (and `type`, where it is not a declared parameter) wraps the result: `{ operator: 'convert', value: <result>, to: <outputType> }`, with `'bool'` spelled `'boolean'`. A computed `outputType` becomes a computed `to`, which is legal, since `to` is an ordinary parameter ("v2 disposition" for `convert` in [v3-operator-parameters-2.md](v3-operator-parameters-2.md)). v2 read `outputType ?? type`, then converted nothing when that was falsy as written ([evaluate.ts:210-211](../../v2-src/evaluate.ts#L210-L211)). So an `outputType` of `null` gives way to `type`, one of `''`, `0` or `false` converts nothing, with no issue, and a computed one that evaluates falsy fails in both (ruled at the PR #191 review).
...
One interaction with batch 3: OBJECT_PROPERTIES turns its `fallback` into `get`'s `default`, which the `convert` then converts, where v2 returned it as it was. A literal fallback already of the target type converts to itself; any other gets a `lossy-default` issue (measured: `fallback: 'N/A'` with `outputType: 'number'` was `'N/A'` in v2 and fails in v3).
...
- **a `$data` reference** with a `fallback` becomes the equivalent `get`, with the fallback as its `default`: batch 3's pairing, since in v2 that fallback caught the missing path;
...
- **Null definitions.** v2 read an alias whose value was `null` as unresolved, so a reference to it gave its own text (`{ $a: null, values: ['$a', 'x'] }` was `'$ax'`), and a fragment argument of `null` did the same. v3's var is `null`. It shows only where a definition is `null`, and goes to the guide.
...
An operator node's undeclared keys, meaning anything that is not a parameter, a property alias, a modifier or an alias definition of its v2 operator, go into a `//` comment object on the result: `{ operator: '+', values: [1, 2], comment: 'adds' }` becomes `{ '//': { comment: 'adds' }, operator: 'plus', values: [1, 2] }`. v2 ignored them, and v3 strips `//` everywhere, so nothing changes and no issue is raised. An author's own `//` key is the comment itself, not one of those keys: `{ operator: '+', values: [1, 2], '//': 'adds' }` becomes `{ '//': 'adds', operator: 'plus', values: [1, 2] }` (ruled at 15.1's fourth chunk). MATCH is the exception, since it read such keys as branches (stage 1, step 7), and so is a call naming a function in `operator`, whose keys went into `input` (stage 1, step 3).

### Values the output discards

A value the output leaves out takes its issues with it: the ones stage 2 raised in converting it, placeholders included, and the ones stage 1 raised inside it, found by its input path. So every issue is about something in the output or about the removal itself, and every placeholder's note is in the output. A node stage 1 left as written already drops what stage 1 found inside it ("Stage 1: normalize").

v2 did not skip every value it did not use, though. It evaluated each of an operator's parameters before it read any, every alias definition, and every argument of a fragment call, so a failure in one failed the node, and any request, query or function call in it ran (measured). So where the output leaves out a value that v2 evaluated and that holds a node or a call, a `lossy-default` issue, `discarded-expression`, says so at the value (ruled at 15.1's seventh chunk):

- the values past the second that GREATER_THAN, LESS_THAN, SUBTRACT and DIVIDE cut, beside `values-cut`;
- SUBTRACT's or DIVIDE's named pair beside `values`, beside `overridden-value`;
- a v2 parameter v3 has no place for: `nullEqualsUndefined`, `excludeTrailing`, STRING_SUBSTITUTION's `substitutionCharacter` in named mode or `numberMapping` in positional mode;
- a malformed BUILD_OBJECT entry, beside `malformed-entry`;
- the alias definitions of a node whose result cannot carry `vars` ("A result that is not a node");
- a fragment argument that the definition does not have and no other argument reads, that loses to its unprefixed spelling, or, with `evaluateFullObject` on, that the body shadows.

A deciding value needs none, since its `deciding-value` already says to rewrite the node by hand, and neither does a value beneath any other placeholder's note. Nor does what v2 never evaluated: the other losers of `overridden-value`, a call-node argument the body shadows, a MATCH branch it never reached, a call's output type it never applied, and a `fallback` that could never answer (measured).
...
- **The wrapper.** `metadata` comes off an operator-node body. `metadata.description` becomes `description`, `metadata.parameters` becomes `parameters`, and `textColor`, `backgroundColor` and any other key go into the definition's `metadata` bag. A `metadata` that is not an object stays on the body, as a key v2 ignored.
- **Declared parameters**, from `metadata.parameters`, keyed by name without the `$`, and renamed where v3 cannot register the name (below):
  - `type`, `description` and `default` carry over, a `default` of `undefined` counting as none, as v2 read it ([evaluate.ts:83](../../v2-src/evaluate.ts#L83)). v2's type names are v3's, apart from `'undefined'`, which is dropped from a union. An unknown type becomes `'any'`, with a `lossy-default` issue, and so does `'undefined'` alone. Any other key on a v2 declaration goes into the parameter's `metadata` bag, since v3's declaration shape is strict.
  ...
  - A parameter declared `required: true` with no default stays required. v2 never enforced `required`: a call that omitted it got the placeholder's own text, `'$name'`. v3 reports `missing-required` instead, and that goes to the guide. Any other parameter is written optional, since nothing else in v2 said it was required. A declared one takes no text default: where no call fills it, v3 reads `null` and v2 gave the placeholder's text, and that goes to the guide.
  - v2 never checked a parameter's type either (measured: `type: 'string', default: 5` gave `6`), and v3 refuses to register a default outside its type, which would block every fragment. So such a parameter takes `'any'`, with a `lossy-default` issue (ruled at 15.1's sixth chunk).
  - **A name without its `$`.** v2 put a declared default into the call's `parameters` under its name as written ([evaluate.ts:81-95](../../v2-src/evaluate.ts#L81-L95)), and spread those over the body, so an unprefixed name never filled a placeholder. Where the name is one the body's operator reads (a parameter, by name or property alias, or a modifier), it set that key: `{ name: 'delimiter', default: ';' }` over a `split` with `delimiter: ','` split on `';'` (measured). That is the canonical v2 of a placeholder at the key, so the key reads `'$delimiter'`, and the default is the declared one, or else the key's own value, which v2 kept where no call set the key. A call's unprefixed `delimiter` is then its argument ("Calls"). Any other unprefixed name is the placeholder the author meant: `{ name: 'name', default: 'friend' }` over a body reading `'$name'` left it unfilled in v2, which gave `'Hello, $name!'`. The converter keeps the declaration, which v3 applies, with an `unprefixed-parameter` issue (ruled at the PR #191 review).
- **Inferred parameters.** v2 did not require declarations: any `"$name"` string in an operator body that no alias definition inside the body resolves was a placeholder a caller could fill (`adder: { operator: '+', values: '$values' }`, with no metadata). Each gets `{ type: 'any', default: '$name' }`, its own text as its default: v2 left a placeholder no call filled as that text, and text such as `'$5.00'` was never a placeholder at all, so the default keeps v2's answer. The default also makes the parameter optional, where v3's default is required (ruled at the PR #190 review). So is a name that a fragment the body calls reads from the body's scope ("Names the body read from its caller", below), which the body does not define: v2 filled it from the body's own caller (measured: with `outer: { operator: '+', values: [{ fragment: 'inner' }, 1] }` and `inner: { operator: '+', values: ['$n', 100] }`, `{ fragment: 'outer', parameters: { $n: 5 } }` gave `106`). Such names are inferred to a fixed point across the fragments, which also ends on the cycles v2 allowed (ruled at 15.1's sixth chunk).
- **A top-level `$name` key on the body** was a default. v2 spread a call's arguments over the body ([evaluate.ts:115](../../v2-src/evaluate.ts#L115)), so a caller's `$name` replaced the body's. It becomes a parameter with that `default`. Beside a declared default for the same parameter, the declared one won, since v2 inserted it into the call's `parameters` (measured: `6`, not `11`), and the body's value goes with `overridden-value`. With `evaluateFullObject` on, v2 read `parameters` through `evaluateObject`, which lifted its `$` keys into the caller's scope, so the body's value won instead (measured: `11`), and the declared default goes.
- **Computed defaults.** v2 evaluated a declared default, and a body's top-level `$name` value, at every call, in the caller's scope (measured: `default: { operator: 'getData', property: 'x' }` gave `8` over `x: 7`), and v3's defaults are constants. A default that converts to a constant is the `default`, unquoted, since a default is never evaluated. A computed one, or one holding a placeholder ("Placeholders"), makes the parameter optional without a default, and the body reads it through v3's own recipe for a computed default, a var at the body's root: `vars: { n: { operator: 'firstOf', values: ['$params.n', <the default>] } }`, with the placeholder read as `'$vars.n'`. An explicit `null` argument then takes the default, where v2 gave the placeholder's text (`'$n1'`, since v2 read a null alias as unresolved), whichever kind the default is, and that goes to the guide (ruled at 15.1's sixth chunk).
- **The body** converts as an expression, with one difference: its placeholders become `$params.name`. Alias definitions below the top level become `vars` as usual. That includes the body's own `fallback` and output type, which v2 evaluated in the caller's scope, where neither the arguments nor the body's top-level `$` keys were (measured: `fallback: '$a'` gave the text `'$a'` beside the argument `$a: 'A'`, and the caller's value under a caller's `$a`). v3's body cannot see its caller, and the quirk is not kept, as the `$count` one is not. That goes to the guide (ruled at 15.1's sixth chunk).
- **A body that is not an operator node**, such as a constant, a string, a plain object or a fragment call, v2 returned as it was ([evaluate.ts:113-114](../../v2-src/evaluate.ts#L113-L114)). It applied no arguments and did not walk the body, whatever `evaluateFullObject` said. So the body becomes `expression` as data: no parameters are inferred, `$name` text stays text, and the `literal` wrap applies as it does with `evaluateFullObject` off. A shorthand body is an operator node when it expands to one, since v2 expanded it first ([evaluate.ts:100](../../v2-src/evaluate.ts#L100)). One difference goes to the guide: a body that is a whole `'$name'` string was replaced by the caller's alias of that name (measured: `6` under a caller's `$n: 5`), and the converted string is text.
- **A null definition**, which v2 allowed ([types.ts:117](../../v2-src/types.ts#L117)), becomes `{ expression: null }`.

v2 could not call a custom function from a body in the `operator` or shorthand form, since it rewrote function calls before it expanded fragments (measured: `Invalid operator`). The converted body calls it, which goes to the guide.
...
- a fragment name that is empty, contains `.`, `[` or `]`, starts with `$`, or is one v3 reserves for registration: a reserved node key, a reference namespace or its one-letter form, or `literal`;
...
The rename is the vars rule: `.`, `[` and `]` become `_` and a leading `$` goes. A name that still clashes, with a reserved key, an operator or another name, gets a numeric suffix (`round_2`), assigned in sorted order so the same fragments always get the same names. `migrateV2Expression` derives the same renames from `V2Options.fragments`, so calls and their arguments follow. The host sees the new names, in `getFragments()` and its editor, so each rename gets a `lossy-default` issue, from `migrateV2Fragments` alone.
...
- **The output** is canonical: `{ fragment: 'name', parameters: { … } }`, or `{ fragment: 'name' }`, v3's zero-parameter call, when there are no arguments.
- **Arguments.** An argument is a key of `parameters`, or a `$` key on the call node itself. v2 spread the call node beneath the body and `parameters` over it, so `parameters` wins where both name one argument, and a body's own top-level `$name` beats a call-node argument but not a `parameters` one (measured: `11` against `3`). Where `V2Options.fragments` holds the definition, a call-node argument the body's top level also defines is therefore dropped, with a `lossy-default` issue. With `evaluateFullObject` on, the body's top-level `$name` beat a `parameters` argument too, since v2 lifted `parameters`' `$` keys into the caller's scope (measured: `11` against `4` with the option off), so that argument is dropped the same way, and the leak into the caller's scope goes to the guide (ruled at 15.1's sixth chunk). Each argument is named without its `$`.
- **An unprefixed key in `parameters`** replaced the body's own key of that name in v2, through the same spread, and not the placeholder. It becomes the argument of that name only where the two are the same: the body's own top-level key of that name holds the whole placeholder, `'$name'`, and nothing else reads it. That covers the usual case, a placeholder named after the parameter it fills, which measures the same. Otherwise it is `non-convertible` (measured: `parameters: { x: 5 }` over `{ operator: '+', values: ['$x', 1], $x: 1 }` gave `2`, since `x` names nothing on `+`, and over a body that read `'$condition'` in two places, `condition: true` left the other one as text). Ruled at 15.1's sixth chunk.
- **An argument the definition does not have**, when `V2Options.fragments` holds the definition, is dropped with a `lossy-default` issue: v2 ignored it, and v3 rejects it as a grammar error. One that another argument reads is kept, as below.
- **Arguments read each other.** v2 spread a call's arguments over the body as the alias definitions of one node, so an argument whose value is a whole `'$name'` that the caller's scope cannot resolve read the call's argument of that name, as a node's definitions read their siblings ("Aliases → `vars`"). A v3 call's arguments cannot see each other, so the one read is bound once in the call's own `vars`, which v3 allows on a call, and each argument reading it becomes `'$vars.name'`: `{ fragment: 'getFlag', $country: '$selected', $selected: <getData> }` becomes `{ fragment: 'getFlag', parameters: { country: '$vars.selected' }, vars: { selected: <get> } }`. It is also the argument of its own name only where the definition has that parameter or is not known, and otherwise goes with no issue, since nothing is discarded. The readers are the arguments the call passes, and the ones they read in turn. The vars are named as a node's are, so none shadows another, and a name the caller's scope defines is read from there instead, as in v2 (measured). An argument that reads a definition at the body's own top level, which the merged node also made its sibling, is not covered. Ruled at 15.2's fourth chunk.
- **Names the body read from its caller.** v2 evaluated a body with the caller's aliases in scope, so an alias defined above the call filled a placeholder the call left empty (measured: `{ operator: '+', $n: 5, values: [{ fragment: 'g' }, 100] }` is `106`, with `g` reading `$n`). That includes the parameters of an enclosing fragment body, since they were aliases too. v3's scopes are lexical, and a body sees only its own parameters. So where `V2Options.fragments` holds the definition, a parameter the call does not supply, which has no default of its own and whose name the call's scope defines, gets that name as its argument: `parameters: { n: '$vars.n' }`, or `'$params.n'` inside a body. It reads the value v2 read (measured the same, for both).
...
- **Modifiers.**
  - `fallback` stays on the call. It caught the body's failures, which v2 evaluated inside the merged node's `try`, but not a failing argument, which was an alias definition of that node, so a data read in the body is beneath it and one in an argument is beneath the next fallback out ("Fallbacks that caught missing data").
  ...
  - The author's `//` is the call's comment. `operator` beside `fragment` was always replaced by the body's own, so it goes with `overridden-value`, as its shorthand form does.
  - Any other key on the call node supplied a body parameter through the spread, and is `non-convertible`.

**The call's output type** is its `outputType`, or else its `type`. A `type` beside the call's own `outputType` goes with `overridden-value`, as on any node. The spread made it the body's, which v2 read as `outputType ?? type` over the merged node, so what it did depends on the body (measured):

| The body                                        | The call's `outputType`                                                                            | The call's `type`                                                                                                                                      |
| ----------------------------------------------- | -------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| an operator node with no output type of its own | the `convert` wrapper                                                                              | the `convert` wrapper, except over PLUS, whose parameter it was (`'string'` over `[1, 2]` gave `'12'`), and over SQL as a rider value: `body-override` |
| with its own `outputType`                       | never applied: `unused-output-type`                                                                | never applied: `unused-output-type`                                                                                                                    |
| with its own `type`                             | applied in place of the body's: the `convert` wrapper, and `replaced-output-type`                  | never applied: `unused-output-type`                                                                                                                    |
| not an operator node                            | never applied, since v2 returned the body first (a body of `42` stayed `42`): `unused-output-type` | never applied: `unused-output-type`                                                                                                                    |

A call on a fragment the options do not hold takes the first row, the common case. `replaced-output-type` is raised where the converted definition applies the body's `type` as a `convert` of its own, which then runs first: `{ fragment: 'f', outputType: 'array' }` over `{ operator: 'count', values: [1, 2], type: 'string' }` is `[2]` in v2 and `['2']` converted, and the two agree wherever the body's conversion loses nothing, such as `'string'` then `'number'`. Ruled at 15.1's sixth chunk, which settled stage 1's exception for a call's `type`.

**Measured the same in both:** arguments in `parameters` and on the call node, a declared default, the shorthand call, an inferred parameter, a node as an argument, a non-operator body, a body-level alias as a default and as overridden, a fragment called inside a body, a fallback on the call, an unprefixed argument, an argument the definition does not have, an argument another reads, and `'$name'` inside a string body. The computed `parameters` is the one difference: `3` in v2, and a type error in v3, where the `$values` key fills nothing.
...
7. **Undeclared keys**: MATCH's go into `branches`, with the precedence in "Batch 3": added to an object, appended as pairs to an array, and left where they are beside a computed `branches` (the exceptions, above). A null `branches` is none, as v2's `branches ?? {}` read it. Any others stay where they are, for stage 2's `//` object.
8. **Fragment arguments.** The call node's `$` keys move into `parameters`, beneath its own keys ("Calls", under "Fragments"). A call with a computed name is left as written (below).
...
- a call-node fragment argument that the body shadows, or that `parameters` also gives, since `parameters` won, and with `evaluateFullObject` on, a `parameters` argument that the body shadows ("Calls");
...
| a computed fragment `parameters`, an unprefixed argument that is not exactly a placeholder's, or another key on the call that v2 spread            | the call, keeping the computed `parameters` and dropping the others | "Fragments"      |
| a call's `outputType` that replaced the body's own `type`                                                                                          | the call, wrapped in `convert`                                      | "Fragments"      |
...
| input nested past the stack's depth, or holding itself, which no JSON config does (ruled at the PR #191 review, so that a conversion never throws) | `literal` of the whole input                                        | "Placeholders"   |

**To the rules, a placeholder is computed.** It stands where v2 evaluated a node, so no rule reads it as the constant it quotes. In a deciding position it is a computed deciding value, a `url` holding one is not v2's `{ url, headers }` object, MATCH binds one once rather than write it twice, and a fragment default holding one is bound in the body like any computed default ("Definitions"), where its note has a node to sit on.
...
| `missing-data-fallback` | the innermost `fallback` above a converted read that has no default of its own: a `$data` reference, a `get`, or an `http` or `graphQL` `returnPath` | `fallback`             | In v2 a missing path failed, and this `fallback` answered. In v3 the read gives `null` and the node carries on. For data, set the host option `strictDataPaths: true` to fail as v2 did, or give the read its own default (`get`'s `default`, or `firstOf`). For a response's `returnPath`, which `strictDataPaths` does not reach, use `firstOf`, or read the path with a `get` and its `default`.                                                                                                                                                  | "Fallbacks that caught missing data" |
| `unprefixed-parameter`  | a declared fragment parameter whose name has no `$` and is not a key the body's operator reads                                                       | the parameter's `name` | v2 read the declared name `{name}` as written, without a `$`, so the declaration never filled the body's `${name}`: its default and type did nothing, and a call that left `${name}` out got that text. v3 applies the declaration. Check the calls that leave it out.                                                                                                                                                                                                                                                                               | "Definitions"                        |
...
| `fallback-converted`         | an OBJECT_PROPERTIES `fallback` beside `outputType`, unless it is a literal already of the target type                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `fallback`                                                                                                                               | This `fallback` becomes `default`, which v3's `convert` then converts to {type}, where v2 returned it as it was. Unless the default is already of that type, it fails or changes. Give a default of that type.                      | "The modifiers"              |
| `malformed-entry`            | a BUILD_OBJECT element with no `key` or no `value`, or the last key of an alternating list of odd length                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | the element                                                                                                                              | v2 skipped an entry with no `key` or no `value`. Removed. For the odd key: An alternating list gives each key a value, and v2 failed on this key, which has none. v3 builds the object without it. Removed.                         | "Batch 3"                    |
...
| `overridden-value`           | a value v2 never read because another spelling won: a shorthand payload's key under another key of the object, an earlier `$` key's result under a later one's, the name a shorthand key resolved to under its payload's own `operator` or `fragment`, a parameter given by name and by alias, a named parameter that `children` fills, `type` beside `outputType` (not on PLUS, nor a rider value on SQL), an explicit SQL `flatten` under the `type` rider, a call-node fragment argument under the same one in `parameters`, a MATCH root branch under the same key in `branches`, SUBTRACT's or DIVIDE's named pair beside `values`, `operator` beside `fragment` on a call, a fragment's top-level `$name` under a declared default for it (the declared default, with `evaluateFullObject` on), or an unprefixed argument's `$` spelling under the unprefixed one | the losing key                                                                                                                           | v2 never read `{key}`: `{winner}` gave the same parameter and won. Removed.                                                                                                                                                         | "Stage 1"                    |
| `discarded-expression`       | a value holding a node or a call that v2 evaluated and the output leaves out: a cut value, SUBTRACT's or DIVIDE's named pair beside `values`, a v2 parameter v3 has no place for, a malformed BUILD_OBJECT entry, the alias definitions of a result that is not a node, or a fragment argument that the definition does not have and no other argument reads, that loses to its unprefixed spelling, or that the body shadows with `evaluateFullObject` on, unless a placeholder's note covers it                                                                                                                                                                                                                                                                                                                                                                       | the value                                                                                                                                | v2 evaluated this and then discarded it, so a failure in it failed the node, and any request, query or function call in it ran. v3 never evaluates it. Removed.                                                                     | "Values the output discards" |
...
| `shadowed-argument`          | a call-node argument, or with `evaluateFullObject` on a `parameters` one, that the body's top level also defines, when the definition is known                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | the argument's key                                                                                                                       | The body of `{fragment}` sets its own `${name}`, which beat this argument in v2, so the argument never applied. Removed.                                                                                                            | "Calls"                      |
| `unknown-argument`           | an argument the definition does not have and no other argument reads, when the definition is known                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | the argument's key                                                                                                                       | `{fragment}` has no parameter `{name}`. v2 ignored the argument, and v3 rejects it. Removed.                                                                                                                                        | "Calls"                      |
...
| `name-renamed`               | a fragment or parameter name v3 cannot register, renamed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | the fragment's key, or the parameter in `metadata.parameters`, or for one not declared, where it was first read or its top-level `$` key | `{name}` cannot be registered in v3 ({the reason}), so it is renamed `{new name}`, and calls follow. Update anything outside expressions that uses the old name.                                                                    | "Definitions"                |
| `unknown-parameter-type`     | a declared fragment parameter whose type v3 does not have, `'undefined'` alone included                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | the parameter's `type`                                                                                                                   | `{type}` is not a v3 type, so the parameter takes `'any'`.                                                                                                                                                                          | "Definitions"                |
| `default-outside-type`       | a declared fragment default that its declared type does not admit                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | the parameter's `default`                                                                                                                | The default `{default}` is not of the declared type `{type}`. v2 never checked it, and v3 would refuse to register the fragment, so the parameter takes `'any'`.                                                                    | "Definitions"                |
| `unused-output-type`         | a call's `outputType` or `type` that v2 never applied, since the body sets its own output type or is not an operator node                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | that key                                                                                                                                 | v2 never applied this `{key}`: {the body of `{fragment}` sets its own output type / the body of `{fragment}` is not an operator node, which v2 returned as it was}. Removed.                                                        | "Calls"                      |
...
| `body-override`              | an unprefixed key in `parameters` that is not exactly a placeholder's argument, a call's `type` over a PLUS body or as a SQL rider, or another key on the call node that v2 spread over the body | that key                                                                   | In v2, `{key}` set the body's own `{key}`, through the call's spread over it. A v3 call passes only arguments. Removed. Make it a parameter of the fragment, or change the body.                                                                                                                                                       | "Calls"               |
...
| `replaced-output-type`       | a call's `outputType` over a body that converts its own result by `type`                                                                                                                         | `outputType`                                                               | In v2 this `outputType` replaced the output type that the body of `{fragment}` sets with `type`. In v3 the body converts its result first, and this `convert` converts that, which gives v2's answer only where the first conversion loses nothing. Check the result, or take the output type off the body.                            | "Calls"               |
...
| `unknown-operator`           | an operator name that is neither v2's nor a listed function's                                                                                                                                    | `operator`                                                                 | `{name}` is not a v2 operator. If it is one of your v2 custom functions, list it in the conversion's `functions` option and convert again, so the call converts to a call on a custom operator of that name. The node is quoted unconverted.                                                                                           | "Stage 1"             |
| `unconvertible-input`        | input the walk cannot finish: nested past the stack's depth, or holding itself                                                                                                                   | the node, or the fragment                                                  | The input nests too deeply to convert, or holds itself, which no JSON config does. It is quoted unconverted. Split it into fragments, or rewrite it by hand.                                                                                                                                                                           | "Placeholders"        |
...
- **Repeats**: a case identical to an earlier one in expression and options, as when a test runs one expression through both of v2's clients, or runs it again to inspect its error. 37 at extraction.
- **Cases on SQLite.** `12_database` runs its SQL tests against Postgres and again against SQLite, with the same expressions apart from one query's dialect. Which database answers is not under test, as which HTTP client asked is not, so the corpus keeps the Postgres cases only (ruled at 15.2's fourth chunk). 16 at extraction.
...
That left 447 cases. Each is an entry of one array:
...
- Postgres, for every SQL case ("Cases on SQLite", above).
...
- **v3** gets `toV3Options` of the case's options, on v3's clients over the same doubles. The runner builds one per distinct options object, which the corpus shares, so most cases share one instance, and clears its result cache before each case, as v2's evaluator is new for each, so no case reads a response an earlier one fetched. Rebuilding from the case's options means v3's merge rules never come into it.
...
- `data` as it is;
- `fragments` through `migrateV2Fragments`, whose issues count toward the cases that call them ("Comparing");
- each function as a v3 operator of its name, the recipe's way: an optional `input` and `...args` as its positional rest, called as v2 called it, `f(input, ...args)`. A name v3 refuses, or one an operator already has (`not`, in the corpus), is not registered, and the summary says so. The calls on it fail in v3, where the converter's issue at each call says the function must be renamed;
...
- **Postgres** has no mock in the v2 tests, which need a live Northwind, so it runs from recordings:
  ...
  - **What the stand-in does.** Its `query` is async, as the real client's is. It resolves with `{ rows }`, the rows as the real client returned them, since the rest of `pg`'s result holds class instances and neither wrapper reads it (ruled at 15.2's first chunk). Or it rejects with a new `Error` each time, since v2's wrapper renames the one it catches, carrying the recorded `message`, `code`, `table`, `column` and `constraint`, the fields v3's failure reads. It accepts only the `query({ text, values })` form, and throws at once on any other, before any promise, such as a `rowMode` or the callback form, so a wrapper that changes how it calls the client fails loudly rather than being answered wrongly.
  ...
  - **Recording.** `pnpm differential --record-sql` runs the Postgres cases once against a live Northwind, through a real `pg` `Client` wrapped to save each result or error. It writes them as a TypeScript module, `differential/sqlRecordings.ts`, which is committed and never hand-edited. Each value keeps the type the real client gave it: a `Date` is written as `new Date('…')`, a `Buffer` (from a `bytea` column) as `Buffer.from('…', 'base64')`, and plain data as literals. A value of any other class stops the recording, so nothing is written as something it was not. The recording is the runner with a live client in place of the stand-in, so it needs no list of queries: it saves whatever either engine asks. That is why the first recording waits for the runner, since v3's queries only exist once a case is converted and run under `toV3Options`.

A request the mock or the recordings cannot answer is caught by the engine that made it, and a fallback may swallow it, so each double also reports it to a listener the runner checks after each engine. What happens next depends on which engine asked (ruled at 15.2's third chunk):

- **A request v2 makes** is the corpus's, so one no double answers means the doubles or the corpus are wrong. It is a runner error, and stops the run, since both engines failing in the same way would otherwise count as a match.
- **A request only v3 makes** is the conversion's doing: the converted expression asked for something v2 never did. It fails v3's outcome, which compares as usual, and the request is printed with the case. At the first run it was one case, #389, whose converted URL kept an alias unresolved, which "Arguments read each other" in "Calls" settled.
...
**The review map**, `differential/reviewed.ts`, records a person's verdict on a ✗, such as `{ 42: 'guide: no implicit coercion' }`, and the case then shows as ⚠ with that note. Together, the issues and the notes are the divergence catalogue: the issues are the converter's half, and the notes are the guide's. Any issue explains a difference, since a result cannot say which node produced it, so a ⚠ is still worth reading when its codes change. That includes the issues of the fragment definitions a case calls, directly or through another fragment's body, which are part of what produced its result, and which the case's line and detail show with its own (ruled at 15.2's third chunk). A ✓ whose conversion raised issues is fine: an issue says the result may differ, not that it does.
...
`Issues raised` counts the cases that raised each code. The summary ends with any v2 function not registered in v3, and why. A ✗ whose v3 made a request no double answers shows it on an `unanswered` line, and one that made any request says on a `requests` line whether both engines sent the same ones.

- **`pnpm differential 42 57`** prints those cases in full, untruncated: the case's options, the v2 expression, the converted expression, every issue with its path and message, both outcomes, any request of v3's that went unanswered, and the requests each engine sent. That is how a case is looked at closely, without opening the corpus.
- **`--all`** adds a ✓ line for every case.
- **`differential/out/differences.md`** is written by every full run, and gitignored: each case that is not ✓, the ✗ first, with its options, the v2 expression, the converted expression, every issue, both outcomes, any unanswered request of v3's and the requests each engine sent. It is the diagnostic material for the review and for debugging later, so the conversions a run produced are kept (added at 15.2's third chunk). A value or an issue list too long to read inline goes to a file of its own beside it, named by case, such as `362.converted.json`.
- **The requests each engine sent**, to the fetch mock and the Postgres stand-in, which report every request to the runner. Where both engines sent the same ones and the outcomes still differ, the conversion asked for the right thing, and the difference is in how a response was read, such as `response-collapse`; where they differ, the conversion changed a request (added at 15.2's fourth chunk). What the two engines' own clients decide, rather than a conversion, is left out of the comparison: the `accept` and `content-type` headers both set to `application/json` (which v2's sends even with no body), the empty `variables` v2 puts in every GraphQL body, and how a URL is spelled, since each is read in its standard form. A body's keys are sorted, and the requests compare in any order, each counted.

### The baseline

`differential/baseline.json` is the accepted result, recorded once the review has done what it can. It holds the v2 version and the totals, then one line per case, ✓ included, with its status and its issue codes, sorted and each once: `"42": "⚠ output-type split-trailing-empty"`. Each case gets its own line, not just a place in the totals, because totals can stay level while cases trade places, and the line says which case moved.
...
The unit suites run in `pnpm test` beside the rest of the v3 suite, one file per part of the converter and three for the differential. Each holds its part to promises this spec makes:
...
| `migrate-issues`       | The catalogue. Only its codes are emitted, each row is triggered with its tag at its path, and every placeholder carries its `//` note, while nothing else does, over the rows and over random input.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `differential-doubles` | The differential's I/O doubles. v2's and v3's HTTP clients get the same outcome from the mocks, the Postgres stand-in answers both engines' wrappers from recordings, and the recorder's module evaluates back to what it recorded, `Date`s and `Buffer`s included. Each double reports what it cannot answer, and the fetch mock and the Postgres stand-in report every request they are sent. `test/live/sql-recording.test.ts` makes the same round trip against a live Northwind (ruled at 15.2's first chunk).                                                                                                                                                                                                                                                           |
...
| `differential-runner`  | Each case's v3 evaluator from `toV3Options`, with v2's functions as operators called `f(input, ...args)`; the comparison, with v2's `undefined` read as JSON writes it and two failures matching; the statuses, with the issues of the fragments a case calls; a request only v3 makes failing its outcome, and one v2 makes stopping the run; the requests each engine sent, read without what the clients decide, and v3's cache cleared before each case; and the baseline's lines and its check in both directions.                                                                                                                                                                                                                                                       |
...
3. **The runner.** The options and `toV3Options`, the comparison and statuses, the output and single-case mode, and `--accept` and `--check`. Then `--record-sql` and the first recording, which need the runner ("I/O"). Tests: `differential-runner`.
