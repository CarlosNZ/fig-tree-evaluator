...
- the root;
- every parameter, modifier and alias definition of a node, since v2 evaluated every parameter;
- the elements of an array in one of those places;
- the values of an object in an `evaluatesContents` parameter;
...
**What it leaves as written.** A node with an unknown operator name, and a node with a computed `children` it cannot map, are kept exactly as the input has them, which v2 trivially evaluates the same. Stage 2 does not enter them, and they become placeholders ("Placeholders", below).
