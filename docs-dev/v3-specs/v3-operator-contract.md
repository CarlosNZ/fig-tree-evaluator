...
Engine-owned regardless of layer: the `cache` option's store/TTL, **failures are never cached**, `trace` records hits and misses. Keys are namespaced by operator name engine-side; a body cannot collide with another operator's entries — and by a **layer tag** as well (Phase 9.1), because `memo` is present on every context, so a `cache: 'auto'` operator's body may call it too and would otherwise be handed the entry the engine wrote for its own outer run.
