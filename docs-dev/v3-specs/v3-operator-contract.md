...
1. **Evaluation-mode vocabulary** — **resolved (September 2026, at Phase-5 implementation)**: the seven modes and their names ship as drafted. Six of the seven are now built (`perElement` follows in Phase 6) and none wanted reshaping: each turned out to be one delivery shape plus the degeneration rule, with no field interactions to trade against. The alternative — fewer modes with qualifier fields — would have had to encode exactly the same distinctions in combinations, and the bodies read as their semantics precisely because the mode says what arrives.
