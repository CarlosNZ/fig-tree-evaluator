...
- **`'auto'`** (default): the engine memoizes the whole body run, keyed on the operator name plus a canonical serialization of the resolved eager parameters. Right for pure operators (where it is rarely on — metadata default `false`) and for any operator a host flips on via the blanket option; zero definition work.
  - *Narrowed at implementation (Phase 9.1, September 2026), because the sentence above is unsound taken literally.* An operator that delivers **any** parameter non-eagerly gets **no auto layer at all**: keying `if` on its eager `condition` alone would let two nodes with one condition and different branches collide, and there is no partial key that is safe. The engine tests the derived `deliversLazily` flag, so a future delivery mode is covered without anyone remembering. A second, value-level handle sweep runs as well, and it is not redundancy: a settlement stream is a plain-prototype object whose only own string-keyed property is `length`, so the key serializer *accepts* it and two different three-element `race` parameters serialize identically — a wrong hit, silently. Where the serializer refuses a resolved value outright (a function, a class instance, a `Map`), the layer is **skipped** — no read, no write, run the body — never degraded to a weaker key.
