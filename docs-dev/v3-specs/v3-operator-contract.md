...
- **Resolution is cancellation.** When the body's returned promise settles, the engine aborts the node's child scope — everything still in flight (undemanded race elements, unevaluated lazy branches mid-flight) is cancelled via the derived abort scope, and a cancelled child raises nothing. This is ledger #7 as a *general rule of the interface*, not an `and`/`or` quirk: any body that returns early gets cancellation free.
  - *Built in Phase 5, not deferred to the kill switch (September 2026 — Carl), so Phase 10 composes the deadline into a chain that already exists rather than retrofitting every node boundary.* Four details the shape depends on. The abort scope covers the node's **parameters only, not its `fallback`** — a fallback runs after the body settled, so under the node's own signal it would be refused at its first node boundary; the *vars* scope spans both, the abort scope does not. **Cancellation is not failure**: it reaches no `fallback`, surfaces under no `mode: 'report'`, and travels the same bail-out as an engine bug for that reason alone — *but it is not invisible: from Phase 12 an abandoned node records a `cancelled` trace status, which is the whole point of having a status distinct from `failed`. Not surfacing as an error and not being observable are different things, and `trace` is where the difference lives.* A **root signal** is kept separate from the node chain, because a scope abort and a caller's abort mean opposite things — silent abandonment versus cutting through every fallback. And an abandoned operand **cannot become an unhandled rejection**, because each element's promise resolves to a `Settlement` and never rejects: the handler is attached where the promise is created, which is the only point early enough (a runtime reports unhandled rejections at the microtask checkpoint, so attaching at abandonment is already too late).
  - *Honest about reach*: JS cannot interrupt code already running, so an abandoned subtree runs to completion and has its result discarded. What the abort reliably does is stop work that has not **started**, and stop anything holding the signal — which is the HTTP/SQL clients, and where the time actually lives.
...
- **Parked failures are the body's Kleene material.** A `race` settlement with `ok: false` is delivered, not thrown; the body fails the node only if the result depends on it (by rethrowing — see below) — the implementation-notes "race with error parking" shape, with the deterministic lowest-index rule as the body's obligation when no decider arrives. `trace` records per-element status engine-side.
  - *Ruled at Phase-12 planning (September 2026, Carl): the same obligation carries `related`.* When no decider arrives and the lowest-index parked failure is raised, the body attaches the other parked failures to it as `related` before throwing — evaluator-methods' one-entry-per-failing-hole rule, which cannot be kept anywhere else, since `decide()` is the only place that ever holds the whole parked pile. Three scopings, each deliberate. It is **`decide()`'s job, not the engine's**: the lowest-index rule is already the body's obligation and this is that rule's other half. It covers **the quantifiers as well as `and`/`or`**, because `some` / `every` are literally the same `decide()` at the other setting, and a rule that applied to one pair and not its twin would be an accident of where it was written. And it fires in **both modes**, `related` being a property of the error rather than of the mode — a thrown error carries its siblings too, where before they were dropped. `collectAll()` is exempt, and structurally so: it raises as soon as an index is *known* lowest, deliberately not waiting on elements whose outcome cannot change the answer, so at the moment it throws there is no sibling pile to attach — recorded here so the asymmetry reads as a consequence rather than an oversight.
...
- Parse-time and runtime **type checking** against the declared types, constraints included (#9); `runtimeTypeCheck: false` degrades exactly the pre-execution layer, per its agreed scope — *at implementation (Phase 4): it removes the type check, the constraints and the derived reject, and nothing else; null-policy enforcement and truthiness are semantics and always run.*
...
- **Boundary normalization**: body result `undefined` → `null`; the **finite-number guard** (#10) converts would-be `NaN`/`±Infinity` results into named runtime failures (code `non-finite-result`) — no more hand-rolled divide-by-zero checks. *Both apply to the body's top-level result only (Phase 4): deep-walking every result would break the O(holes) cost model.*
...
*Resolved (Phase 9.1, September 2026): the split ships as drafted.* This section was flagged as the contract's most implementation-sensitive spot, and the build moved its **edges** — the `deliversLazily` exclusion, the refusal rule, the layer tag — without moving the shape. `memo` did not prove awkward: the I/O bodies each call it once, around exactly the work that can fail on the wire, and `returnPath` sitting outside the key fell out rather than needing arranging. The two alternatives recorded here stay rejected, and for the reason drafted: a definition-level `cacheKey()` would still need a second post-cache shaping hook, and engine-generic caching would re-introduce raw-spelling keys for exactly the operators ledger #16 fixed.
...
*Built (Phase 13.1, September 2026), as drafted. The exported shapes are `OperatorInfo` / `ParameterInfo` (assembled in `src/introspect.ts`), the capability flags are `hasValidate` and the reported `cache` mode, and `restParam` / `timeoutParam` travel while `deliversLazily`, `evaluate` and the brand do not. One reading recorded, on the required-field check rather than the snapshot: `category` is **collected, not gated** — `defineOperator()`'s fast-fail gate exists for the fields without which nothing else is checkable, and a missing category blocks no other check, so it reports beside every other violation.*
...
4. **Unset delivery as absent key** — **confirmed as drafted (Carl, September 2026, Phase-4 implementation)**. The layered chain fills a declared default *before* delivery, so a body's destructuring default can only ever apply where the declaration has no default of its own — which is that body's business. Doc line, no lint. The inferred params type makes such keys optional (`?:`).
