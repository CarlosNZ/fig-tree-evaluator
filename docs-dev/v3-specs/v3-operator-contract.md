...
| `readsOptions` | `string[]` | — | option blocks the body reads (ledger #17); the named blocks arrive frozen on `context.options`, and blocks it doesn't name are simply not on that object. **Not a restriction** — a definition declares its own access, so any operator may name any block: the field records an intention, it does not police one. Whether it earns its keep at all is open Q10 |
...
10. **`readsOptions` — does declared options access earn its keep?** (opened September 2026, at Phase-4 implementation; Carl unconvinced, deciding later.) The security reading is dead, and the prose that implied it is corrected above: a definition declares its own access, so the field polices nothing — an operator that wants a block simply names it. Two justifications remain, and the first is weaker than it looks (Carl's objection, recorded): **introspection** — `getOperators()` reports the declaration, but its primary consumer is the editor, which needs an operator's parameters and types to present a UI and has little use for which option blocks a body reads internally; and the **first-class principle** — core `http` gets host-mutable config through options (`baseEndpoint` / `headers`, rotatable via `updateOptions`), so a custom operator must be able to as well, which means *some* delivery of option blocks to bodies has to exist. So the real question is narrower than "keep or drop the capability": it is whether the **declaration** is required, or whether bodies should just receive the whole frozen options object. The alternatives: (a) keep as drafted; (b) drop the field and pass all options frozen — one less field to learn, and the silent-`undefined` failure of a forgotten declaration goes with it; (c) derive access by convention, an operator seeing the block matching its own name — works for every core operator, but strands a custom operator needing a block named for something else, so it fails the first-class principle; (d) host-side declaration at registration, the only variant with real teeth, at the cost of ceremony at every registration site for a threat model v3 does not otherwise defend against. Wiring is out of scope either way: clients are closures, never options (Options § Operator registration). If this resolves to drop, the follow-through is ledger #17 ([v3-operator-parameters.md](v3-operator-parameters.md)), the `http`/`graphQL` options-access note in [Part 2](v3-operator-parameters-2.md), `context.options` in § Runtime interface, and `src/operatorDefinition.ts` / `src/evaluate/context.ts`.
