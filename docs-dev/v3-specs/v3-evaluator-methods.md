...
*Settled at Phase-12 planning (September 2026), by spiking the stack before anything was built on it — the drafted shape ships unchanged, and two readings are corrected.* **No `const` type parameters are needed, and they would be actively harmful.** The worry the spike went looking for was `trace?: boolean` widening `{ trace: true }` to `{ trace: boolean }`, which would make the `{ trace: true }` arm of `ResultShape` unreachable; TypeScript's `const` modifier was the obvious answer. It turns out there is nothing to fix: widening applies to a mutable variable declaration, not to inference into a type-parameter position, so a fresh object literal argument infers `{ trace: true }` on its own. Adding `const` instead infers `{ readonly trace: true }` and — the real cost — turns the `operators` array literal into a **readonly tuple** (`readonly [Defn[], Defn]`), which the declared mutable array type does not accept. So the class and the method take plain type parameters. The twelve-case matrix that proves it (instance-level, per-call, and per-call overriding the instance in both directions) becomes the type-level test. **The recorded escape hatch does not fit this signature**: `evaluate`'s type parameter is the *options*, not the return type, so `fig.evaluate<EvaluationResult>(…)` is not writable — a caller who needs to force the shape asserts on the result instead. One limitation stands and is inherent rather than chosen: a caller who hoists the options into a variable (`const opts = { trace: true }`) does get the widening, and the instance types as bare-value; `const` would not have saved that case either, since it only reaches literal arguments.
...
*Built (Phase 13.2, September 2026), with the shape as drafted and three readings. The lossless renderer **is** the deduplication key — bracket-quoting any key the dot grammar cannot carry makes `renderSegments` injective, so a path's canonical identity is just its render and there is no second notion to keep in step; `parsePath(renderSegments(s)) === s` is the property asserted over the awkward keys. The method **parses without the static-check pass**, which only appends issues: holes and dependencies are pass-1 products, so the second walk would be work nobody reads. And traversal order has one counter-intuitive consequence in exactly the case that motivated the segment record: a single key holding a dot sorts **after** the two-level read it used to collide with (`first.last`, then `["first.last"]`), because the comparison reaches `'first'` before `'first.last'` at segment 0.*
...
11. **What report mode is *for*** — **resolved: production resilience**, the draft's premise throughout (contested at review, confirmed at close-off — § Intended use records the intent, § Why the filler is `null` records the resolution). The hole-filler stays `null`; the mode **name stays `report` with a possibly-improve marker** (`'partial'` / `'resilient'` floated — revisit deliberately before v3.0 ships); a separate authoring/debug mode with loud in-band markers stays a possible later addition (seed use case: the dry-run row in § Intended use). *Revisited at Phase-12 planning (September 2026, Carl) — the deliberate revisit the marker asked for, taken at the moment the mode was about to be built, which is the last point before the name reaches code. `report` **stays**, and the marker is discharged rather than carried further: `'resilient'` names the posture and `'partial'` names the return, but neither is enough better to be worth diverging from every cross-reference in these docs, and the name is not what was ever unclear — the intent was, and that is recorded. Renaming stays free while v3 is unreleased.*
