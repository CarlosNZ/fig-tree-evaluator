...
**`isEvaluable(expr)`** — the v2 instance method `isFigTreeExpression` renamed, because deep evaluation changed the question: _everything_ is a valid v3 expression (any JSON evaluates), so "is this a FigTree expression" no longer discriminates. The question that remains meaningful — the one the editor was really asking — is "does this instance's parse find anything to evaluate?": `true` iff the parse finds at least one evaluable hole, or any static error (a malformed node _engaged_ the grammar — it is an expression, a broken one; classifying it as inert data would be the silent de-invocation failure mode the sibling-key rule was designed against). Never throws. The v2 _structural_ export's fate (`isOperatorNode` or nothing) belongs to Packaging. _Built (Phase 13, September 2026): both halves are tested, `artifact.holes.length > 0 || any error-severity issue`. The Phase-13 planning reading that the two halves were one test — the parser classifies a malformed node as **evaluable, never constant**, so a sibling-key violation is a hole and an unrecognized `$` key (inert data with a warning) is not — holds for node-level malformation but not for a structural key: `{ vars: 'high' }` raises its error from the `vars` compiler and folds to a constant with no hole, so the issue stream is consulted after all (found at Phase-13 review). Pass 1 alone suffices, since the static-check pass finds issues only on operator nodes and fragment calls, which are holes already. **Normalization is not evaluation** (ruled at Phase-13 review, Carl): a `//` comment key, an `undefined` value or a `vars` block nothing reads all make the output differ from the input, and all answer `false`, because none of them is an expression — so the question is "anything to evaluate", not "non-identity", and the summary table reads the same way. Like `getDependencies()`, it compiles fresh rather than going through the parse cache, for the same per-keystroke churn reason._

**`clearCache()`** — **new, agreed at close-off** (all-or-nothing — no per-operator filter; the store interface is where finer control lives): empties the operator-result memo store (the `cache` option's store). The concrete case: the host knows external state changed (a DB write landed, a config was republished) and wants the next evaluation fresh without waiting out `maxTime` or constructing a new instance. Does _not_ touch the parse cache, which is semantically transparent (keyed on input identity against a stable registry — there is never a correctness reason to clear it; `updateOptions` invalidates it on any change to `operators`, `fragments` or `operatorDefaults`, the three options the compile artifact consumes — see the implementation notes' invalidation-set entry). v2's `getCache()`/`setCache()` die: persistence and inspection are the pluggable `cache.store`'s business — a host that wants to serialize its cache supplies a store it holds a reference to. _Built (Phase 9.1, September 2026): the method is total across every store, which `store.clear()` alone cannot make it. `CacheStore` gained `delete` and `clear` so the engine can do what this entry promises at all, but a `clear()` may return a promise while this method is specified **sync**, and a request may already be in flight when it lands. So the store holds engine-owned envelopes stamped with a **generation counter**: `clearCache()` bumps it and fires `clear()`, and from that moment any envelope written under a lower generation reads as a miss — which also discards the result of a request that was computed before the clear and completed after it. The counter is per instance, so the guarantee is total for what this instance (or any instance at or below its generation) wrote; in a store shared between instances, an envelope another instance wrote under a higher generation stays reachable until `store.clear()` lands. The parse cache is untouched, as specified, and structurally cannot be reached: it has no clearing API._
...
| `updateOptions(options)`                                    | **Kept**                                   | agreed merge semantics (Options); throws on registration errors (Fragments); re-validates the whole registry, and invalidates the parse cache on the **presence** of `operators`/`fragments`/`operatorDefaults` — not on a deep comparison of their values (Phase 8.1) |
