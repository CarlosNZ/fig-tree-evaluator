/**
 * The shapes `convertV2ToV3` returns ("`./convert` — the module surface" in
 * docs-dev/v3-specs/v3-migration.md). Types only, exported from the root, so
 * the `fig-tree-evaluator/convert` subpath stays a function module with no
 * type surface of its own ("Types" in docs-dev/v3-specs/v3-packaging.md).
 */
